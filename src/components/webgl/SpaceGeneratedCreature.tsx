...
export function SpaceGeneratedCreature({ artwork, index }: SpaceGeneratedCreatureProps) {
  ...
  const orbitParams = useMemo(() => {
    const baseRadii = [4.5, 6.2, 8.0, 9.8, 11.5, 13.5];
    const orbitRadius = baseRadii[index % baseRadii.length];
    const orbitSpeed = 0.32 * Math.pow(orbitRadius / 4.5, -1.5);
    const inclination = (index % 5 - 2) * 0.2;
    const phaseOffset = index * 1.618 + Math.random() * Math.PI * 2;
  ...
  useFrame(({ clock }, delta) => {
    ...
    const CREATURE_ORBIT_CENTER = new THREE.Vector3(0, 0, -8.85);
    const orbitAngle = t * orbitParams.orbitSpeed + orbitParams.phaseOffset;
    ...
    const tangent = new THREE.Vector3(
      ...
      cosA * sinInc * orbitParams.orbitRadius,
    ...
    const depthScale = THREE.MathUtils.mapLinear(
      THREE.MathUtils.clamp(group.position.z, -23.0, 5.0),
      -23.0,
      5.0,
      0.65,
      1.25
    ...
    group.scale.setScalar(depthScale * Math.max(0.7, 1.0 - index * 0.04));
