...
1. `App` 检查 URL：`/admin` 进入管理后台，`/submit` 进入手机拍照/上传页，其他路径加载星河主页。三个页面使用路由级懒加载，手机端不会下载 WebGL 主场景代码。
2. 主页创建 WebGL 画布、相机、轻量天空和控制面板。
3. 如果配置了后端地址，前端分页读取全部有效历史作品并写入 `useArtworkStore`；随后每 2.5 秒只通过现有作品接口读取最新 1 条记录和总数，生成轻量版本号。版本没有变化时不下载完整列表，也不写入 Zustand 或触发 WebGL 重渲染。
...
1. 手机访问同一前端服务的 `/submit`，选择“拍摄画作”调用后置相机，或选择“从相册选择”。
2. 页面只接受 JPG、PNG、WebP，单张最大 15 MB；选择后先显示预览、文件名和大小。
3. 确认提交后复用 `submitArtworkFile`、AI 特征识别和 TripoSplat 长轮询，但关闭本地粒子回退。如果后端失败，手机明确显示失败，不会把只存在手机内存中的作品误报为已经入屏。
4. 生成完成后，有效作品总数、最新作品 ID 和创建时间组成的版本号发生变化。
5. 大屏下一次轻量轮询通过 `GET /api/artworks?limit=1&offset=0&status=active` 发现版本变化，才分页读取完整作品库，并把新增后端 ID 合并进 `useArtworkStore`。
6. 合并完成后调用 `beginSpotlight`。模型自身确认可渲染后进入现有聚光队列，避免在资源尚未加载时强行切换镜头。

```text
手机 /submit
  → 拍照或相册选图
  → POST /api/artworks
  → GET /api/jobs/{jobId} 长轮询
  → 后端生成完成并写入 SQLite
  → 大屏 GET /api/artworks?limit=1
  → 版本变化时才 GET /api/artworks
  → 差异合并 Store
  → 新模型聚光入场
```

轮询使用请求互斥，同一时刻最多执行一次同步；页面隐藏时暂停定时轮询，重新可见后立即检查。`BroadcastChannel` 和 `storage` 事件仍用于同一电脑的后台操作，并可强制执行一次完整同步。
