...
class TripoSplatPipeline:
    def __init__(self, ckpt_path: str, decoder_path: str, dinov3_path: str,
        ...
        if self._device.type == "cpu":
            cpu_dtype_name = os.getenv("TRIPOSPLAT_CPU_DTYPE", "float32").lower()
            cpu_dtype = torch.bfloat16 if cpu_dtype_name == "bfloat16" else torch.float32
            encoder_dtype = cpu_dtype
            model_dtype = cpu_dtype
        else:
            encoder_dtype = torch.bfloat16
            model_dtype = torch.float16

        self.dinov3      = load_dinov3      (dinov3_path,             device=self._device, dtype=encoder_dtype)
        self.vae_encoder = load_vae_encoder (flux2_vae_encoder_path,  device=self._device, dtype=encoder_dtype)
        self.rmbg        = load_rmbg        (rmbg_path,               device=self._device, dtype=model_dtype)
